...
void output(const Data *d) {
    if (d)
        cout << d->iField << " " << d->sField << " " << d->dField << endl;
    else
        cout << "NULL" << endl;
}
...
// use size() and operator[] to access data elements in contai1ner
void createIndex(DATATYPE dt, const Container &c) {
    ...
    if (dt == DATATYPE::STR) {
        for (int i = 0; i < c.getSize(); i++) {
            String.insertElem(c[i]);
        }
    }
    if (dt == DATATYPE::DBL) {
        for (int i = 0; i < c.getSize(); i++) {
            Double.insertElem(c[i]);
        }
    }
}
...
void notify(OPCODE op, const Data *value) {
    if (OPCODE::INS == op) {
        Int.insertElem(value);
        String.insertElem(value);
        Double.insertElem(value);
    }
    if (OPCODE::DEL == op) {
        Int.deleteElem(value);
        String.deleteElem(value);
        Double.deleteElem(value);
    }
}
...
Data *searchByStr(const string &value) {
    return String.search(value);
}

Data *searchByDbl(double value) {
    return Double.search(value);
}
